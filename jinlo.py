...
local_tz = pytz.timezone("Asia/Seoul")  # Change this to your local time zone
...
def on_change():
    ...
    previous_input = st.session_state.get("previous_input", "")

    # 현재 입력이 이전 입력과 다른지 확인합니다.
    if user_input and user_input != previous_input:
        ...
        if openai.api_key:
            ...
            now = datetime.now(local_tz).strftime("%H:%M")  # Get the current time in the local time zone
...
if not st.session_state.get("initial_question_displayed", False):
    ...
    st.session_state["chat"] = st.session_state.get("chat", []) + [("🙇", datetime.now(local_tz).strftime("%H:%M"), initial_question)]
...
if not openai.api_key:
    st.error("OpenAI API 키를 입력하세요. *-입력하지 않을 시 답변이 제한됩니다.-*")
else:
    if st.button("질문"):
        if question:
            # 채팅을 시각화하기 위해 질문 내용 저장
